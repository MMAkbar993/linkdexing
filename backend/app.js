...
// Behind Nginx every request arrives from 127.0.0.1, so without this the
// rate limiters (and API request logs) would treat every visitor as one
// client - ten logins site-wide would lock everyone out. Exactly 1 hop, not
// `true`: trusting more would let a client spoof X-Forwarded-For to dodge
// rate limits.
...
app.use("/api/v1/coupons", require("./src/api/v1/coupons/routes"));
app.use("/api/v1/index-check", require("./src/api/v1/indexcheck/routes"));
app.use("/api/v1/dev", require("./src/api/v1/dev/routes"));
